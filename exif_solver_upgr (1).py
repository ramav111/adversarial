...
class ExifSolver(object):
    ...
    def setup_net(self, net):
        ...
        if self.init_summary:
            self.train_writer = tf.compat.v1.summary.FileWriter(os.path.join('./tb', self.exp_name + '_train'), self.sess.graph)
    ...
    def setup_summary(self):
        ...
        if not self.net.freeze_base:
            self.summary.extend([tf.compat.v1.summary.scalar('exif_loss', self.net.loss),
                                 tf.compat.v1.summary.scalar('exif_accuracy', self.net.accuracy)])
        if self.net.train_classifcation:
            self.summary.extend([tf.compat.v1.summary.scalar('cls_loss', self.net.cls_loss),
        ...
        if self.use_exif_summary:
            ...
            self.individual_summary = {tag:tf.compat.v1.summary.scalar('individual/' + tag, self.tag_holder[tag]) for tag in self.net.train_runner.tags}
        return
